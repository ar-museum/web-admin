...
function delete_trivia(obj)
{
    ...
    bootbox.prompt('Scrie "STERGE" pentru a confirma actiunea', function(result) {
        ...
        {
            $.ajax({
                url: 'trivia/delete/' + _triviaId,
                type: 'DELETE',
                success: function(r) {
                    var _pos = TRIVIA.fnGetPosition($(obj).closest('tr').get(0));
                    TRIVIA.fnDeleteRow(_pos);
                    TRIVIA.fnDraw(false);
